...
def check_case(bad_value):
    ...
    sympy_lhs = lhs.subs(x, bad_value)
    sympy_residual = simplify(sympy_lhs - 1)

    z = complex(float(bad_value), 0.0)
    reciprocal_z = complex(float(1 / bad_value), 0.0)
    oracle_lhs = cmath.sqrt(z) * cmath.sqrt(reciprocal_z)
    oracle_residual = oracle_lhs - 1
    ...
    detail = (
        ...
        f"sympy_lhs={sympy_lhs} sympy_residual={sympy_residual} "
        f"oracle_lhs={oracle_lhs} oracle_residual={oracle_residual}"
    )
    return ok, detail
