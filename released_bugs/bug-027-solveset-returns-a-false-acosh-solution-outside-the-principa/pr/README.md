# Fix solveset false solution for solveset returns 1 for acosh(x) = 2*pi*I outside the principal acosh range

## Summary

`solveset(Eq(acosh(x), 2*pi*I), x, domain=S.Complexes)` returns a set containing a value that does not satisfy the original equation. The solver is inverting a principal inverse function as if its elementary inverse were global.
