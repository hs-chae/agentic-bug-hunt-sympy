...
def check_case(a):
    ...
    poles = [a - 1, a + 1]
    values = [expr.subs(x, p) for p in poles]
    denominator_values = [(((p - a)**2)**(1/3) - 1) for p in poles]
    ok = all(s.contains(p) is S.true for p in poles)
    detail = f"a={a} singularities={s} poles={poles} values={values} oracle_denominators={denominator_values}"
    return ok, detail
